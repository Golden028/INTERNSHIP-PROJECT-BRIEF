...
<div id="confirmDeletePhotoModal" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[10000] flex items-center justify-center p-4 transition-all duration-300">
    <div class="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-gray-100 transform scale-95 transition-all duration-300 space-y-4">
        <div class="flex flex-col items-center text-center space-y-3">
            ...
            <div class="space-y-1">
                <h4 class="text-base font-bold text-gray-900">Hapus Foto Profil?</h4>
                <p class="text-xs text-gray-500 leading-relaxed">Apakah Anda yakin ingin menghapus foto profil ini dan kembali menggunakan inisial nama awal bawaan sistem?</p>
            </div>
        </div>
        <div class="flex items-center gap-3 pt-2">
            <button type="button" onclick="closeDeletePhotoModal()" class="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold py-2.5 rounded-xl border border-gray-200 transition focus:outline-none">
                Batal
            </button>
            <form action="{{ route('profile.delete_photo') }}" method="POST" class="w-full m-0">
                @csrf
                @method('DELETE')
                <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-2.5 rounded-xl transition shadow-md shadow-red-600/10 focus:outline-none">
                    Ya, Hapus Foto
                </button>
            </form>
        </div>
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl hover:border-emerald-500 transition-colors cursor-pointer relative bg-gray-50/50">
                        <div class="space-y-2 text-center pointer-events-none">
                            ...
                            <p class="text-[10px] text-gray-400 font-medium">Mendukung semua resolusi berkas gambar (Maks. 20MB)</p>
                        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col justify-between">
            ...
            <div class="p-6 flex-1 flex flex-col items-center justify-center space-y-5">
                <div class="relative group select-none">
                    <div onclick="openUploadModal()" class="cursor-pointer">
                        @if(isset(auth()->user()->avatar) && auth()->user()->avatar != null)
                            <img src="{{ asset('avatars/' . auth()->user()->avatar) }}" class="w-24 h-24 rounded-full object-cover shadow-md ring-4 ring-emerald-50 transition-all duration-300 group-hover:scale-105">
                        @else
                    ...
                    <div class="absolute inset-0 bg-slate-900/50 text-white rounded-full opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all duration-300 scale-100 group-hover:scale-105 shadow-md pointer-events-none">
                        <div onclick="openUploadModal()" class="text-center cursor-pointer pointer-events-auto">
                            <i class="fa-solid fa-camera text-lg block mb-0.5 animate-pulse"></i>
                            <span class="text-[8px] font-bold uppercase tracking-wider block">Ganti Foto</span>
                        </div>
                    </div>

                    @if(isset(auth()->user()->avatar) && auth()->user()->avatar != null)
                        <button type="button" onclick="openDeletePhotoModal(event)" class="absolute -bottom-1 -right-1 bg-red-600 hover:bg-red-700 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-lg border border-white transition-all z-20 focus:outline-none" title="Hapus Foto Profil Saat Ini">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden lg:col-span-2 flex flex-col justify-between">
            ...
            <div class="p-6 flex-1">
                <form id="profileCredentialForm" action="{{ route('profile.update') }}" method="POST" class="space-y-5 m-0">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Nama Lengkap</label>
                        <input type="text" id="credentialName" name="name" required max="100" value="{{ old('name', $user->name) }}" class="w-full mt-1.5 p-2.5 border border-gray-300 rounded-lg text-sm focus:outline-emerald-600 placeholder-gray-400 font-medium text-gray-800">
                    ...
                    <div>
                        ...
                        <input type="email" id="credentialEmail" name="email" required max="150" value="{{ old('email', $user->email) }}" class="w-full mt-1.5 p-2.5 border border-gray-300 rounded-lg text-sm focus:outline-emerald-600 placeholder-gray-400 font-medium text-gray-800">
                    ...
                    <div class="border-t border-gray-100 pt-4">
                        <p class="text-xs text-emerald-800 font-medium bg-emerald-50/70 p-3 border border-emerald-200/50 rounded-lg mb-2 flex items-start gap-2.5">
                            ...
                            <span>Biarkan kolom kata sandi di bawah ini **tetap kosong** jika Anda tidak berniat untuk mengganti password log masuk sistem saat ini.</span>
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="password" id="credentialPassword" name="password" placeholder="Isi hanya jika ingin ganti..." class="w-full mt-1.5 p-2.5 border border-gray-300 rounded-lg text-sm focus:outline-emerald-600">
                        ...
                        <div>
                            ...
                            <input type="password" id="credentialPasswordConfirmation" name="password_confirmation" placeholder="Ulangi input sandi baru..." class="w-full mt-1.5 p-2.5 border border-gray-300 rounded-lg text-sm focus:outline-emerald-600">
                        </div>
                    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // KELOLA POP-UP: Tanpa komparasi string, murni menangani animasi fade-out jika session resmi ada
        if (successNotif) {
            setTimeout(() => {
            ...
            }, 3000); // Bertahan utuh selama 3 detik sebelum menghilang halus
        }
    });

    // SISTEM MODAL POP-UP UPLOAD FOTO PROFIL
    ...
    // SISTEM MODAL POP-UP KONFIRMASI HAPUS FOTO
    function openDeletePhotoModal(event) {
        event.preventDefault();
        event.stopPropagation();
        const modal = document.getElementById('confirmDeletePhotoModal');
        modal.classList.remove('hidden');
        modal.classList.add('flex');
    ...
    // Penutupan Modal Otomatis Saat Klik Luar Area Area Kotak Putih
    window.addEventListener('click', function(event) {
        const uploadModal = document.getElementById('uploadPhotoModal');
        const deleteModal = document.getElementById('confirmDeletePhotoModal');
